...
**A startup guide and What's new**
A fresh install now opens with a short "Welcome to CapForge" walkthrough: seven cards covering what the app is for, the library, transcription, editing and styling, export, the Publish workspace and connecting Claude, with a link straight to the settings each one talks about and to the tutorial video. After an update, a "What's new" card lists the highlights of every version released since the one you last saw, newest first, and links to the full changelog. Each appears once, only on the library screen — never over a running transcription or your editor — and both can be reopened any time from Settings → General, which also shows the version you are running. An existing install that has never seen either prompt gets the release notes rather than the walkthrough. The tutorial video plays inside the guide when you ask for it, and opens in your browser when you would rather watch it there; nothing is loaded from YouTube until you click.
