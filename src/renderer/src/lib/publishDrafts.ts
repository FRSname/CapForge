...
import type { PostsDraft } from './publishPosts'
import { applyPostsDraft, survivingPosts } from './publishPosts'

/** A field's draft value: what the record holds, except `posts`, whose draft is a delta. */
export type DraftValue<K extends PublishFieldId> = K extends 'posts'
  ? PostsDraft
  : PublishAuthored[K]

/** The drafts map: every authored field, optional. */
export type PublishDrafts = { [K in PublishFieldId]?: DraftValue<K> }
 ...
 * only the frames routes may change it. A `localized` or `posts` draft is a
 * delta, laid over the record's languages or posts rather than replacing them.
 */
export function mergeDrafts(record: PublishRecord | null, drafts: PublishDrafts): PublishAuthored {
  ...
  const localized = drafts.localized
    ...
    : {}
  const posts = { posts: applyPostsDraft(base.posts, drafts.posts) }
  return { ...merged, ...thumbnail, ...localized, ...posts }
}
 ...
 * `posts` is judged per channel and field (`survivingPosts`).
 */
export function survivingDrafts(
  ...
  for (const [field, value] of Object.entries(drafts)) {
    ...
    if (id === 'posts') {
      const kept = survivingPosts(drafts.posts ?? {}, local.posts, remote.posts, locked === 'posts')
