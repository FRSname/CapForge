 ...
 * mounted in the tree.
 *
 * A11y: focus is trapped inside the dialog while a render is busy. Escape
 * deliberately does NOT cancel — aborting an in-flight render is destructive,
 * so it requires an explicit activation of the Cancel button.
 * The status message is a polite live region so milestones are announced;
 * the rapidly-updating percentage is deliberately NOT live (it would spam
 * screen readers on every tick).
 */
...
export function RenderProgressModal({ render }: Props) {
  ...
  useFocusTrap(cardRef, render.busy)
