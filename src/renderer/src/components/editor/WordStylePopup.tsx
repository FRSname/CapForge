...
export function WordStylePopup({
  ...
  const [hlRadius, setHlRadius] = useState(
    overrides.highlight_radius ?? defaults.highlightRadius ?? 0
  )
  ...
  const [ulColor, setUlColor] = useState(
    overrides.underline_color ?? defaults.underlineColor ?? defaults.activeColor
  )
  ...
  function buildOverrides(): WordOverrides {
    ...
    if (effectiveTransition === 'underline') {
      ...
      if (ulColor !== (defaults.underlineColor ?? defaults.activeColor))
        next.underline_color = ulColor
    }
