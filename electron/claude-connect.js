...
/**
 * Build the stdio server entry. Both `cwd` and `env.PYTHONPATH` point at the
 * folder containing `mcp_server/`: macOS python honours PYTHONPATH, while the
 * Windows embedded python ignores it (its `._pth` puts cwd on sys.path), so we
 * set both to cover every client/platform combo.
 */
