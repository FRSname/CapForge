...
const fs = require('fs')
const { PythonBackend, PREFERRED_PORT } = require('./python-manager')
...
app.whenReady().then(async () => {
  ...
  // Every channel must exist before the window does — a fast renderer that
  // invokes an unregistered channel gets "No handler registered", not a retry.
  registerIpcHandlers()

  // Window first, backend in parallel. uvicorn needs ~0.5s to answer and the
  // first paint no longer waits for it; `backend:port` / `backend:local-token`
  // gate on PythonBackend.whenReady(), so the renderer still never talks to a
  // backend that isn't up.
...
/**
 * Await backend readiness, tolerating a start failure. `start()` already showed
 * the error dialog for that case, so the caller answers from the snapshot and
 * the renderer reports a connection error instead of waiting forever.
 */
async function backendReadyOrFailed() {
  if (!pythonBackend) return
  try {
    await pythonBackend.whenReady()
  } catch {
    // Reported by the startup dialog above — nothing to add here.
  }
}

/**
 * Register every `ipcMain.handle` channel. Called once from `app.whenReady()`,
 * before `createWindow()` and before the backend has started.
 */
function registerIpcHandlers() {
  ...
  // IPC: get backend port. The window opens before the backend is reachable,
  // so this resolves only once uvicorn has answered its health check — that is
  // what keeps the renderer from caching the PREFERRED_PORT guess. If the
  // backend FAILED to start, whenReady() rejects and we still answer with the
  // snapshot, so the renderer surfaces a connection error instead of hanging.
  ipcMain.handle('backend:port', async () => {
    await backendReadyOrFailed()
    return pythonBackend ? pythonBackend.port : PREFERRED_PORT
  })

  // Per-launch token the renderer must send on the local media endpoints
  // (/api/serve-audio, /api/video-info). Generated by PythonBackend and injected
  // into the backend env; the renderer echoes it back as a ?token= query param.
  // Same readiness gate as backend:port — an empty token here would 401 every
  // media request for the rest of the session.
  ipcMain.handle('backend:local-token', async () => {
    await backendReadyOrFailed()
    return pythonBackend ? pythonBackend.localToken : ''
  })
