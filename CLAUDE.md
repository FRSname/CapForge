...
- **Overlay MOV alpha convention**: the transparent overlay export writes **premultiplied** alpha in the ProRes 4444 branch of `_render_overlay` (`-vf premultiply=inplace=1`, `video_render.py`) because ProRes 4444/QuickTime is a premultiplied-alpha convention and Premiere/FCP un-premultiply on import — straight alpha would render semi-transparent caption backgrounds washed out in the NLE. The WebM overlay branch stays **straight** alpha (VP9/browser convention) — do not touch it. Pinned by `backend/tests/test_overlay_alpha.py`. Diagnostic recipe to simulate an NLE import: `[1:v]unpremultiply=inplace=1[u];[0:v][u]overlay`.
