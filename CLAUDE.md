...
- **Overlay MOV alpha convention**: the transparent overlay export writes **premultiplied** alpha in the ProRes 4444 branch of `_render_overlay` (`-vf premultiply=inplace=1`, `video_render.py`) because ProRes 4444/QuickTime is a premultiplied-alpha convention — straight alpha risks mis-compositing in NLEs (note: Premiere may still auto-detect the clip as "Straight Alpha"; users should conform it to Premultiplied). The WebM overlay branch stays **straight** alpha (VP9/browser convention) — do not touch it. Pinned by `backend/tests/test_overlay_alpha.py`. Diagnostic recipe to simulate an NLE import: `[1:v]unpremultiply=inplace=1[u];[0:v][u]overlay`. **Known non-file failure mode**: a washed-out semi-transparent caption box over video in Premiere with a *correct* file is Premiere compositing in linear color (sequence "Composite in Linear Color" / linearized CM working space) — verify with a gamma-space ffmpeg composite before touching the encoder (diagnosed 2026-07-16, `docs/plans/overlay-opacity-persists-round2.md`).
- **BT.709 color tagging convention**: all user-facing encodes except WebM (overlay MOV, overlay MP4, baked MP4) force the RGB→YUV conversion to BT.709 limited range (`scale=out_color_matrix=bt709:out_range=tv,format=…` — in the MOV branch `premultiply` must stay FIRST in the `-vf` chain, it operates in RGBA) **and** tag the stream (`_BT709_TAGS` in `video_render.py`) so NLEs stop guessing. Matrix and tags must always change together — one without the other reintroduces a hue shift on saturated colors (pinned by the saturated-color test in `test_overlay_alpha.py`). The MOV branch additionally needs `-movflags write_colr` (experimental flag; without it the mov muxer writes no `colr` atom for ProRes at all) and only `color_space` is ffprobe-visible there — re-verify on any FFmpeg upgrade.
