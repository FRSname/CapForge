...
- **Pillow shapes through HarfBuzz** (`ImageFont.Layout.RAQM`, chosen once as `LAYOUT_ENGINE` in `video_render.py` when libraqm can load FriBiDi) so a font's OpenType features — contextual alternates (`calt`), ligatures (`liga`/`clig`), GPOS kerning — apply as Chromium applies them to the preview and to the HTML layer. Pillow's `BASIC` engine applies **none** of them (cmap lookup plus the legacy `kern` table), which is how Pricedown's extended glyphs showed in the preview but not in the file (issue #73). The app puts FriBiDi beside the managed interpreter (`electron/text-shaping.js`); a process without it logs `Text shaping: … BASIC` at startup and renders plain glyphs. Non-zero `tracking` is the one path that defeats shaping *everywhere*: the tracked pen walk draws one character at a time in Canvas and Pillow alike, so no alternate or ligature can fire there — the same in both, hence not a delta.
...
- Tracked text (`tracking ≠ 0`) is drawn one character at a time, so contextual alternates and ligatures cannot fire — in Canvas and Pillow alike, by construction.
