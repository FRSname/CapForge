...
- **Phase 4 — felt demo:** the live in-app run (Studio → connect Claude → place + save + apply + native captions → render) remains manual (needs the app + whisperx venv + a connected agent). Remaining smaller follow-ups: persist placed effects in project save/load (`ProjectFile` has no `effects` field today — placing effects then Save→Open loses them; clean to add to `App.tsx` handleSave/restore + `project.ts`); draggable effects track on the canvas timeline.
