...
- **Phase 3 follow-up — portrait/4K/square fit for native captions** ✅ `09155c0`. Native registry caption components are authored for a fixed 16:9 stage; HyperFrames renders a sub-composition at its OWN body size **unscaled** (verified by frame grab: a 1920×1080 caption dropped top-left into a portrait frame, clipped). `fit_caption_component(path, W, H)` rewrites the component's viewport+body to the target canvas and fits its content with one CSS transform on the comp root (`[data-composition-id]`: scale-to-width, bottom-center origin) — component-agnostic, JS/layout untouched, no-op at native size. Proven end-to-end: portrait full-width bottom-anchored captions, 4K uniform 2×, 1080p byte-stable. **Key HyperFrames mechanics (the hard-won bits):** (a) a sub-comp renders at its body/viewport size, not the host div's; (b) a CSS transform on the *host div* in the parent does NOT scale the sub-comp (dead end — captions vanished); (c) so the fix must live inside the component. 126 tests.
- **Follow-up — persist placed effects in project save/load** ✅ `f38f3b8`. `ProjectFile` gains `effects?: EffectClip[]`; App save/autosave merge App-owned effects onto the gathered payload; restore (Open + crash-recovery) applies `setEffects(file.effects ?? [])`. No backend sync needed (render path sends effects in the body; AgentLiveSync only refetches on a broadcast, so restore isn't clobbered). vitest +3 round-trip/back-compat tests.
- **Phase 4 — felt demo:** the live in-app run (Studio → connect Claude → place + save + apply + native captions → render) remains manual (needs the app + whisperx venv + a connected agent). Remaining smaller follow-up: draggable effects track on the canvas timeline (today effects are placed/edited numerically in the EffectsPanel).
