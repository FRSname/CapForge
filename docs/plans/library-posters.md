...
- **Where it runs** — `LibraryStore` takes an `on_created` hook, told once per record *minted* (never on a fingerprint hit), and `router.get_store` wires it to `posters.start_grab`; that is the one seam every creation path shares — the create route, project import, the studio migration, the agent's `load_video`/`transcribe` — so none of them has to remember to ask. Startup calls `posters.start_backfill` (bounded to 200 records, missing media skipped, a removed record skipped rather than aborting the pass) so libraries from before #6, and records whose grab failed, catch up. Both run on the module's single-worker pool: one ffmpeg at a time, never on the caller's thread, a failure logged rather than lost in a Future. (The first cut scheduled the grab as a FastAPI `BackgroundTasks` step in two routes; review found the agent's `load_video` and `migrate-studio` bypassed it until the next launch.)
