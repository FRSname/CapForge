...
- **Known gaps / polish:** `render_hyperframes` uses default caption styling (not the live StudioSettings — agent could read `get_ui_state` and pass config); agent-added effects aren't yet mirrored into the EffectsPanel UI; `update_effect` tool not added (only add/remove/replace).
