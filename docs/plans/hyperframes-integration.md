...
**Slice 1 ✅ DONE (2026-06-19):** Two text-based effect types — `lower_third` (title/subtitle bar, accent rule, slide-in) and `kinetic_stat` (big value + label, pop-in). The generator was refactored to a per-type design: a positioned outer `.fx` + an animated inner element, so each type carries its own enter/exit and the centering transform never fights the motion (the same split that fixed caption drift). `logo` preserved. Semantic detection: `find_semantic_moments(kind)` in `engine/moments.py` for `numbers` (contiguous numeric words merge; digit tokens detected), `cta` (phrase-beats-word), `speaker_change` (first word of each diarized run). Agent action space: `list_effect_types` advertises all three types + variable schemas; `add_effect` supports text variables with per-type default anchors; new `find_semantic_moments` MCP tool + `/api/agent/find-semantic-moments` endpoint + client method; README tools table updated. **Proven end-to-end:** 101 backend+MCP tests pass; a generated project with all three effects lints **0/0**; draft render + frame grabs confirm the stat ("25% / GROWTH"), the lower-third ("Jane Doe / CEO, Acme" with accent bar), and the logo all composite over the video alongside the active-word caption.
**Slice 2 ✅ DONE (2026-06-19):** Two more effect types — `highlight` (translucent highlighter marker, `scaleX` sweep from the left via GSAP `transformOrigin`) and `b_roll` (timed image, sized at an anchor or `fullscreen` cover, sitting behind the captions at z-index 5). Asset-copy factored into `_copy_asset` (shared by logo + b_roll); `canvas_h` threaded for resolution-relative marker sizing. Agent action space: `list_effect_types` + `add_effect` extended (highlight `color`/`width`/`height`; b_roll `src`/`width`/`fullscreen`) with per-type default anchors; README updated. **UI:** `EffectsPanel` rebuilt for all five types — a `+` button per type (logo/b_roll open the image picker), per-effect editor with type-specific fields (title/subtitle, value/label, marker width/height, b-roll width + fullscreen). `EffectClip.type` widened to `EffectType`; `mapEffect` already maps agent-placed types through. **Proven end-to-end:** 105 backend+MCP tests pass; renderer typecheck clean; a generated project with **all five** effect types lints **0/0**; draft render + frame grab shows logo, b-roll, kinetic-stat, and the highlight marker compositing together over the video with captions on top.
- **Remaining in Phase D:** richer `find_moments` heuristics; optional caption-word-coupled highlight (today's highlight is anchor-placed, not bound to a specific caption word). Manual: the live agent run ("name bar when a new speaker starts", "counter when they mention a number").
