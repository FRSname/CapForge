...
- **Remaining for Phase B (UI):** effects track on `useTimeline` (draggable EffectClip blocks), an effect inspector StudioCard (logo file + position + timing), frontend `effects` state sent in the render body + mirrored to backend UI-state for the agent, and project save/load persistence.
- **Verify (UI):** add a logo at t=X in the UI → "Render with HyperFrames" → logo animates at X; persists in project save/load.
