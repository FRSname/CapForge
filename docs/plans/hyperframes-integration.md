...
- **Remaining in Phase D:** `highlight` (caption-word-coupled emphasis) and `b_roll` (timed/fullscreen image) effect types; UI "+ Add" buttons for the new types in `EffectsPanel`; richer `find_moments` heuristics. Manual: the live agent run ("name bar when a new speaker starts", "counter when they mention a number").
